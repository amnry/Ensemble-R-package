...
bagged_model <- function(X, y, r.bagging, modelType) {
  ...
  for (i in 1:r.bagging) {
    ...
    if (modelType == "linear") {
      ...
      # Convert fitCoefs to a numeric vector, replacing "." with NA
      fitCoefs_numeric <- as.numeric(as.matrix(fitCoefs))
      terms <- which(fitCoefs_numeric != 0)
      #fitCoefs_numeric[fitCoefs_numeric == "."] <- NA

      # Check for non-missing and non-zero coefficients
      #non_zero_indices <- which(!is.na(fitCoefs_numeric) & fitCoefs_numeric != 0)

      # Extract the non-zero coefficients
      #terms <- fitCoefs_numeric[non_zero_indices]
